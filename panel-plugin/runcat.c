...
static gint runcat_get_cpu_delay(RunCatPlugin *rc) {
    ...
    if (rc->cpu_usage < 20) return 100;
    if (rc->cpu_usage < 30) return 90;
    if (rc->cpu_usage < 40) return 90;
    if (rc->cpu_usage < 50) return 80;
    if (rc->cpu_usage < 60) return 80;
    if (rc->cpu_usage < 70) return 70;
    if (rc->cpu_usage < 80) return 70;
    if (rc->cpu_usage < 90) return 60;
    return 50;                             // 20 FPS
}
